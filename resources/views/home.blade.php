				...
				<article class="post bg-white p-6 rounded-lg shadow-md">
					<header class="mb-4">
						<h2 class="text-2xl font-bold text-gray-800 hover:text-blue-600">
							<a href="{{route('blog',[$blog->id])}}">{{$blog->blog_title}}</a>
						</h2>
						...
						<div class="meta flex items-center mt-2 text-sm text-gray-400">
							...
							<a href="#" class="flex items-center space-x-2">
								...
								<span class="text-gray-600 "><a class="hover:text-gray-900" href="{{route('dashboard',[$blog->user->id])}}">{{$blog->user->first_name}} {{$blog->user->last_name}}</a></span>
							</a>
						</div>
					</header>
					...
					<footer class="mt-4 flex justify-between items-center">
						<a href="{{route('blog',[$blog->id])}}" class="text-blue-500 hover:underline">Continue Reading</a>
						<div class="flex space-x-4 text-gray-500">
							@php
							$current = currentUser();
							if($current){
								$isSaved = $current->savedBlogs()->where("blog_id",$blog->id)->exists();
							}else{
								$isSaved = false;
							}
